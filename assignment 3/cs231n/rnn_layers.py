...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    # introducing non linearity to derived features
...
def rnn_step_backward(dnext_h, cache):
    ...
    # gradient for non linearity
    d_tanh = dnext_h*(1 - next_h**2)
    # grad wrt x
    dx = np.dot(d_tanh, Wx.T)
    # grad wrt prev hidden state
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    N, T, D = x.shape
    H = h0.shape[1]
    # initializing h, cache
    cache = []
    h = np.zeros((N, T, H))
    # itereting through all timesteps
    for i in range(x.shape[1]):
      h[:,i,:], c = rnn_step_forward(x[:,i,:], h0, Wx, Wh, b)
      # updating h0 to the current h
      h0 = h[:,i,:]
      cache.append(c)
...
def rnn_backward(dh, cache):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    # initializing last step
    last_step = dh.shape[1]-1
    # storing grad of last step
    dprev_h = dh[:,last_step,:]
    # iterating backwards through timesteps
    for i in range(last_step, -1, -1):
      # suming grad of hiddne states
      if i != last_step:
        dprev_h += dh[:,i,:]
      # RNN backprop 
      dx_step, dprev_h, dWx_step, dWh_step, db_step = rnn_step_backward(dprev_h, cache[i])
      # initializing grad matrices
      if i == last_step:
        dx = np.zeros((dx_step.shape[0], dh.shape[1], dx_step.shape[1]))
        dh0 = np.zeros((dprev_h.shape[0], dprev_h.shape[1]))
        dWx = np.zeros((dWx_step.shape[0], dWx_step.shape[1]))
        dWh = np.zeros((dWh_step.shape[0], dWh_step.shape[1]))
        db = np.zeros((db_step.shape[0]))
      # storing the grad values from RNN backward
      dx[:,i,:] = dx_step
      dh0 = dprev_h
      dWx += dWx_step
      dWh += dWh_step
...
def word_embedding_forward(x, W):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    # looking corresponding rows in W
    out = W[x] 
    cache = (W, x)
...
def word_embedding_backward(dout, cache):
    ...
    W, x = cache
    dW = np.zeros_like(W)
    # iterrating over batch and sequence dimensions
